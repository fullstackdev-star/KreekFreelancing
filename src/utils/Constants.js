...
const Constants = {
    ...
    if (Constants.isDriver()) {
      if (!global.curUser.car) {
        if (showAlert)
          alert(
            'No Car Profile',
            'Please submit car profile and wait approving from Rydedeal.',
          );
        initRoute = 'Setting';
      } else {
        if (!global.curUser.approved_at) {
          if (showAlert)
            alert(
              'Not Approved',
              'Your driver account is not approved yet, please wait until approved, If you want to change some information, please check setting.',
            );
          initRoute = 'Setting';
        } else {
          if (!global.curUser.paymentMethods) {
            ...
            if (showAlert)
              alert(
                'No BillingMethod',
                'Please add billing method and set default method to purchase subscription.',
              );
          } else if (!global.curUser.defaultPaymentMethod) {
            initRoute = 'BillingMethod';
            if (showAlert)
              alert(
                'No Default Billing Method',
                'Please set or add default billing method to purchase subscription.',
              );
            ...
            if (showAlert)
              alert(
                'No Payment Verified',
                'Please verify your payment method to get money from ride directly.',
              );
            ...
            if (global.curUser.is_subscription_ended === true) {
              ...
              if (showAlert)
                alert(
                  'No Subscription',
                  'Please purchase subscription to accept ride.',
                );
            }
          } else {
            initRoute = 'driver_ride_browse';
  ...
  curUserCity: () => {
    return global.curUser &&
      global.curUser &&
      global.curUser.cities &&
      global.curUser.cities.length > 0
      ? global.curUser.cities[0]
      : null;
  },
