...
import ma from '../assets/lang/ma.json';
import vt from '../assets/lang/vt.json';
...
const Constants = {
  ...
  titleInvite: 'Invite your friends',
  msgInvite: 'Ozenii Massage Mobile',

  GoogleApiKey: 'AIzaSyCZr7rhFfp4NBdJSMoNBK64VXIJU7bL6xo',

  // OneSignalAppID:"a2e7224c-d1d6-4463-8210-17a32e77954a", // from developer account
  OneSignalAppID: 'fb0cd936-1dcd-4743-8c0c-dee135fc1430', // from owner account

  keys: {
    curLang: 'cur_lang',
    curAddress: 'cur_address',
    curLat: 'cur_lat',
    curLng: 'cur_lng',
  ...
  PUSH_TestPush: 98,
  PUSH_PostFindStaff: 101,
  PUSH_BidOnReq: 102,
  PUSH_BookNewReq: 103,
  PUSH_AcceptReq: 104,
  PUSH_CancelReqByStaff: 105,
  PUSH_CancelReqByCustomer: 106,
  PUSH_BookReqTimeOver: 107,
  PUSH_RejectReqFromStaff: 108,
  PUSH_CancelReqByAdmin: 109,
  PUSH_TimeOverFindStaff: 110,
  ...
  getFormattedDateString: (date) => {
    let formatted_date =
      Constants.appendLeadingZeroes(date.getDate()) +
      '-' +
      Constants.appendLeadingZeroes(date.getMonth() + 1) +
      '-' +
      date.getFullYear();
  ...
  getOverSecondsFromTimeStamp(stamp) {
    let nowTimeStamp = Constants.getCurrentTimeStampInSeconds();
    let diff = nowTimeStamp - stamp;

    return diff;
  },

  getDateTimeFromStampInSeconds(stampSecs) {
    let stampDate = new Date(stampSecs * 1000);

    return stampDate.toLocaleString();
  },

  crashlyticsException: (exception) => {
    if (Platform.OS === 'android') {
      Crashlytics.logException(
        'Android Crash Exception:' + JSON.stringify(exception),
      );
    } else if (Platform.OS === 'ios') {
      Crashlytics.recordError(
        'iOS Crash Exception:' + JSON.stringify(exception),
      );
    }
  },
  ...
  trans: (keyStr) => {
    if (global.lang === 'en') {
      ...
      return keyStr + ' not defined in ' + global.lang;
    }
    if (global.lang === 'ma') {
      if (keyStr in ma) {
        return ma[keyStr];
    ...
    return 'No ' + global.lang;
  },

  setDefaultLang: () => {
    switch (global.curLang) {
      case 'English':
        global.lang = 'en';
        break;
      case 'Vietnamese':
