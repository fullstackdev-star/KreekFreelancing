...
class CProfessionalsFavoritesScreen extends React.Component {
  static contextType = NavigationContext;

  constructor(props) {
    super(props);

    console.log('CProfessionalsFavoritesScreen start');
  ...
  onRefresh = (type) => {
    ...
    RestAPI.get_favorite_professional_list(params, (json, err) => {
      if (type == 'init') {
      ...
      if (err !== null) {
        Alert.alert(
          ...
          'Failed to get favorites from server.',
        );
      } else {
        if (json.status === 1) {
...
export default CProfessionalsFavoritesScreen;
