...
class CMRequestsInvitesListScreen extends React.Component {
  static contextType = NavigationContext;

  constructor(props) {
    super(props);

    console.log('CMRequestsInvitesListScreen start');
    ...
    RestAPI.get_all_invites_list(params, (json, err) => {
      ...
      if (err !== null) {
        ...
        if (json.status === 1) {
          ...
          if (type == 'more') {
            let data = itemDatas.concat(json.data.invite_list);
            this.setState({itemDatas: data});
          } else {
            this.setState({itemDatas: json.data.invite_list});
          }
        } else {
...
export default CMRequestsInvitesListScreen;
