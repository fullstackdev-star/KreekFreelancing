...
class FCConfirmPaymentScreen extends React.Component {
  constructor(props) {
    super(props);

    console.log('FCConfirmPaymentScreen start');

    this.state = {
      purchaseAmount: 0,
      accessCode: global._accessCode,
  ...
  componentDidMount() {
    ...
    switch (global._membershipPlan) {
      case 'Basic':
        purchaseAmount = 0;
        break;
      case 'Basic+':
        purchaseAmount = 10.95;
        break;
      case 'Professional':
        purchaseAmount = 20.95;
        break;
      case 'Business':
        purchaseAmount = 29.95;
        break;
      case 'Executive':
    ...
    if (global._isMonthly === false) {
      purchaseAmount *= 12;
    }
    this.setState({purchaseAmount: purchaseAmount});
  }
      ...
      <>
        ...
        <View style={[GStyles.rowEndContainer, {marginTop: 30}]}>
          <Text style={GStyles.regularText}>
            {global._membershipPlan} (1 month Free Trial)
          </Text>
          <Text style={GStyles.regularText}>${purchaseAmount}</Text>
        </View>
        <View
          style={[
            GStyles.rowEndContainer,
            GStyles.borderBottom,
            {height: 60, marginTop: 64},
          ]}>
          <Text style={GStyles.mediumText}>Total</Text>
          <Text style={GStyles.mediumText}>${purchaseAmount}</Text>
        </View>
      </>
    );
  };
  ...
  _renderButton = () => {
    return (
      <TouchableOpacity onPress={this.onNext} style={{marginVertical: 40}}>
...
export default FCConfirmPaymentScreen;
